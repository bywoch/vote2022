...
mapSet = {
    // 초기화 메서드
    init: function () {
        // SVG 맵과 텍스트 표시 영역을 선택
        $map = $('#svg-wrap');
        $txt = $('#area-txt');

        // 고급 모드 여부에 따라 파티 번호 설정
        if (adv_bol) {
            party_num = party.split("_");
        }

        // SVG 데이터, 맵 정보, 파티 색상 정보의 개수 설정
        svg_cnt = svg.length;
        map_cnt = info.length;
        party_cnt = party_color.length;

        // 맵 그리기, 텍스트 설정, 데이터 매칭 메서드 호출
        this.mapdrow();
        this.txtset();
        this.dataMaching();
    },
    ...
    // SVG 맵의 각 지역에 대한 텍스트 정보를 설정하는 메서드
    txtset: function () {
        var code = "";
        ...
        // 생성된 텍스트 정보를 HTML에 적용
        $txt.html(code);
    },
    // SVG 맵에 대한 데이터를 매칭하여 색상과 투표율을 설정하는 메서드
    dataMaching: function () {
        $txt.removeAttr('class');
        $('#area-txt > span').addClass("empty");

        // SVG 데이터에 대한 매칭 작업 수행
...
$(function () {
    // 저사양 브라우저가 아닌 경우에만 실행
    if (!$('html').hasClass('low-browser')) {
        // 지도 타입 선택 버튼에 대한 이벤트 핸들러 등록
